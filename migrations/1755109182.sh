...
# Users can still opt-in to Cloudflare DNS using: archwrld-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-archwrld-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-archwrld-dns.network
  ...
  echo "To use Cloudflare DNS, run: archwrld-setup-dns Cloudflare"
fi