if command -v limine &>/dev/null; then
  sudo tee /etc/mkinitcpio.conf.d/archwrld_hooks.conf <<EOF >/dev/null
...
#timeout: 3
default_entry: 2
interface_branding: ArchWRLD Bootloader
...
EOF
  ...
  if [[ -z ${ARCHWRLD_CHROOT_INSTALL:-} ]]; then
...
if [[ -n $EFI ]] && efibootmgr &>/dev/null && ! efibootmgr | grep -q ArchWRLD &&
